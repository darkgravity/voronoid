// ═══ GLSL Shader Sources ═══
// Auto-extracted from shader.js for modular loading
...
uniform int       uShapeBlendMode;      // per-shape blend mode (-1=off, 0-9)
uniform float     uShapeBlendHueOff;    // per-shape hue offset for hue-shift blend
uniform float     uShapeBlendMagnitude; // per-shape strength slider (0-1)
uniform int       uCellColorMode;       // 0=cell color, 1=custom, 2=pass-through
uniform vec3      uCustomCellColor;     // solid color when cellColorMode==1
uniform int       uFinalPass;           // 1=last pass in chain, apply dither
uniform sampler2D uOrigSceneTex;        // always the original scene (not chain)
uniform sampler2D uCellGradTex;         // gradient lookup for Cell Color = Gradient mode
...
vec2 quadtreeGrid(vec2 rawUV, vec2 baseGrid) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        float lum = dot(texture2D(uOrigSceneTex,
...
BlockInfo getBlock(vec2 rawUV) {
    ...
    // Force-square BEFORE quadtree so subdivision uses a square base —
    // otherwise rect cells (e.g. 300×1) make adjacent rows pick different
    // subdivision levels and the force-squared result fragments.
    if (uForceSquare == 1) gridSize = vec2(max(gridSize.x, gridSize.y));
    vec2 actualGrid = quadtreeGrid(rawUV, gridSize);
    // Snap cell to integer px
    vec2 cellPx = actualGrid;   // already integer from quadtreeGrid (and already square if forceSquare)
    vec2 off = pixPos - canvasCenter;
...
void main(){
    ...
    // Original pixelated cell color from the TRUE scene — luminance effectors always read from this
    vec3 origCellColor = texture2D(uOrigSceneTex, uv).rgb;

    // Cell color source (per-shape): 0=default (scene cell color), 1=custom, 2=gradient lookup by luminance
    if(uPixelate==1 && uPassthrough==0) {
        if(uCellColorMode==0) color = origCellColor;
        else if(uCellColorMode==1) color = uCustomCellColor;
        else if(uCellColorMode==2) {
            float cgLum = clamp(dot(origCellColor, vec3(0.299, 0.587, 0.114)), 0.0, 1.0);
            color = texture2D(uCellGradTex, vec2(cgLum, 0.5)).rgb;
        }
    }

    // pixelMagnitude: for non-image shapes = strength slider directly (cell color shows fully at 1.0)
    // (image shapes overwrite this with their own image-luminance × opacity)
    float pixelMagnitude = uShapeBlendMagnitude;
    ...
    if(uPixelate==1 && effectiveShape>0){
        ...
        if(effectiveShape==9 && uImgPixelEnabled==1){
            ...
            float lum = dot(origCellColor, vec3(0.299, 0.587, 0.114));
            ...
            if(ipMsk2==1){
                ...
                // Normal mode: image luminance × opacity = pixelMagnitude
                // The shape-level blend mode will apply this later
                pixelMagnitude = dot(imgSample.rgb, vec3(0.299,0.587,0.114)) * ipOp2;
        ...
        // Alpha luminance effector: modulate shape opacity by cell luminance
        if(uSdfAffectAlpha==1) {
            shapeAlpha *= clamp(mix(uSdfMinAlpha, uSdfMaxAlpha, cellLum), 0.0, 1.0);
        }
                ...
                if(diaAlpha > 0.001){
                    ...
                    if(uImgPixel2Enabled==1){
                        ...
                        if(dInner.x > 0.005 && dInner.x < 0.995 && dInner.y > 0.005 && dInner.y < 0.995){
                            float dLum = dot(origCellColor, vec3(0.299,0.587,0.114));
    ...
    if(uPixelate==1) {
        vec3 accum = texture2D(uAccumTex, rawUV).rgb;
        vec3 layer = (uShapeBlendMode >= 0)
            ? blendVH(accum, color, uShapeBlendMode, uShapeBlendHueOff)
            : color;
    ...
    /* ── Instance blend: composite whole instance onto previous instances ──── */
    ...
    // Dither to break 8-bit banding — only on final pass to avoid accumulation
